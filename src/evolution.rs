...
impl EvolutionEngine {
    ...
    pub fn combine(&self, parent1: &Genome, parent2: &Genome) -> Genome {
        ...
        let all_concepts: Vec<String> = parent1
            .concepts
            .iter()
    ...
    pub fn fitness(&self, voxel: &Voxel) -> f64 {
        ...
        // Emotion balance
        let emotion_balance = 1.0
            - (voxel.emotion_valence.abs()
                + voxel.emotion_arousal.abs()
                + voxel.emotion_dominance.abs())
                / 3.0;
    ...
    pub fn evolve(&self, voxels: &mut [Voxel]) {
        ...
        let mut fitness_scores: Vec<(usize, f64)> = voxels
            .iter()
            .enumerate()
        ...
        for i in top_count..voxels.len() {
            ...
            if rng.gen_bool(self.crossover_rate) {
                // Crossover
                let mut new_genome =
                    self.combine(&voxels[parent1_idx].genome, &voxels[parent2_idx].genome);
