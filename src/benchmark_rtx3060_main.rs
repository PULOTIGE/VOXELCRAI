// Benchmark runner for RTX 3060 in 4K with baked lighting patterns
use winit::{
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let (config, pattern_name) = if args.len() > 1 {
        match args[1].as_str() {
            "light" => {
                ...
                (BenchmarkConfig::rtx3060_4k_light(), "Sunny")
            }
            "heavy" => {
                println!("Using Heavy benchmark preset (Stress Test)");
                (BenchmarkConfig::rtx3060_4k_heavy(), "Stormy")
            }
            _ => {
                println!("Using Standard benchmark preset (RTX 3060 target)");
                (BenchmarkConfig::rtx3060_4k(), "Sunny")
            }
        }
    } else {
        println!("Using Standard benchmark preset (RTX 3060 target)");
        println!("Usage: cargo run --bin benchmark-rtx3060 [light|heavy]");
        (BenchmarkConfig::rtx3060_4k(), "Sunny")
    };
    ...
    let window_attributes = Window::default_attributes()
        .with_title("RTX 3060 4K Benchmark - Baked Lighting Patterns")
